...
from models.utils import calculate_loss

from data.loader import get_dataloader, recover_original_prediction
...
class ARNNModel:
    ...
    def predict(self, data, future_steps=1): # for real time predictions
...
class XGBoostModel(): # one-dimensional?
    ...
    def train_validate_forward_dataloader(self, train_data, num_epochs=None): # how many epochs?? works with batch_Size > 1???
        ...
        all_inputs = np.array(all_inputs).reshape(len(all_inputs), -1).tolist() # wouldnt it be more efficient better with a numpy array?
